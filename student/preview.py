...
def collect_images(img_dir, pixiv_dir=None):
    """收集目录下所有图片文件"""
    paths = []
    for ext in IMAGE_EXTS:
        paths.extend(glob.glob(os.path.join(img_dir, f'*{ext}')))
        paths.extend(glob.glob(os.path.join(img_dir, f'*{ext.upper()}')))
    if pixiv_dir and os.path.exists(pixiv_dir):
        for ext in IMAGE_EXTS:
            paths.extend(glob.glob(os.path.join(pixiv_dir, f'*{ext}')))
            paths.extend(glob.glob(os.path.join(pixiv_dir, f'*{ext.upper()}')))
...
def main():
    ...
    output_dir = args.output_dir or os.path.normpath(os.path.join(SCRIPT_DIR, '..', 'outputs', 'model_previews'))
    img_dir = args.img_dir or os.path.normpath(os.path.join(SCRIPT_DIR, '..', 'extracted_imgs', 'imgs'))
    pixiv_dir = os.path.join(img_dir, 'pixiv_imgs')
